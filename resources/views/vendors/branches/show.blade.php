    ...
    <div class="max-w-4xl mx-auto mt-10 px-6">
        ...
        <div class="flex justify-between items-center mb-8">
            <h2 class="text-3xl font-extrabold text-gray-800 flex items-center">
                <i class="fas fa-store mr-3 text-purple-600"></i> {{__('Branch Details')}}: {{ $branch->name }}
            </h2>
        ...
        <div class="bg-white rounded-xl shadow-lg p-6">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="space-y-4">
                    ...
                    <p class="text-sm text-gray-600 flex items-center">
                        ...
                        <strong>{{__('Address')}}:</strong> <span class="ml-1">{{ $branch->address }}</span>
                    </p>
                    
                    <p class="text-sm text-gray-600 flex items-center">
                        <i class="fas fa-map-pin mr-2 text-purple-500"></i>
                        <strong>{{__('Latitude / Longitude')}}:</strong> 
                    ...
                    <p class="text-sm text-gray-600 flex items-center">
                        ...
                        <strong>{{__('Phone')}}:</strong> <span class="ml-1">{{ $branch->phone ?? 'N/A' }}</span>
                    </p>
                    
                    <p class="text-sm text-gray-600 flex items-center">
                        <i class="fas fa-envelope mr-2 text-purple-500"></i>
                        <strong>{{__('Email')}}:</strong> <span class="ml-1">{{ $branch->email ?? 'N/A' }}</span>
                    </p>
                    
                    <p class="text-sm text-gray-600 flex items-center">
                        <i class="fas fa-user-tie mr-2 text-purple-500"></i>
                        <strong>{{__('Manager')}}:</strong> <span class="ml-1">{{ $branch->manager_name ?? 'N/A' }}</span>
                    </p>
                    ...
                    <p class="text-sm text-gray-600 flex items-center">
                        ...
                        <span class="ml-1 font-semibold">
                            @if ($branch->is_active && $branch->is_approved)
                                <span class="text-green-500 bg-green-100 px-2 py-1 rounded-full">{{__('Active & Approved')}}</span>
                            @elseif (!$branch->is_approved)
                                <span class="text-yellow-500 bg-yellow-100 px-2 py-1 rounded-full">{{__('Pending Approval')}}</span>
                            @else
                                <span class="text-red-500 bg-red-100 px-2 py-1 rounded-full">{{__('Inactive')}}</span>
                            @endif
                        </span>
                    </p>
                ...
                <div class="flex justify-center items-center">
                    ...
                    @else
                        <div class="h-48 w-48 flex items-center justify-center bg-gray-100 rounded-lg shadow-md">
                            <span class="text-gray-500 text-sm">{{__('No Photo Available')}}</span>
                        </div>
                    @endif
                </div>
            </div>
            ...
            <div class="mt-6 flex justify-end space-x-4">
                <a href="{{ route('vendors.branches.edit', $branch->id) }}" class="inline-flex items-center px-4 py-2 bg-blue-600 text-white font-semibold rounded-lg shadow-md hover:bg-blue-700 transition duration-300 transform hover:-translate-y-1">
                    <i class="fas fa-edit mr-2"></i> {{__('Edit Branch')}}
                </a>
            </div>
        </div>
    </div>
