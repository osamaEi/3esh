    ...
    <div class="max-w-4xl mx-auto mt-10 px-6">
        ...
        <div class="flex justify-between items-center mb-8">
            <h2 class="text-3xl font-extrabold text-gray-800 flex items-center">
                <i class="fas fa-user-edit mr-3 text-purple-600"></i> {{__('Edit Employee')}}
            </h2>
            ...
            <form action="{{ route('vendors.employees.update', $employee->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                ...
                <div>
                    <label for="password" class="block text-sm font-medium text-gray-700 mb-1">
                        <i class="fas fa-lock mr-1 text-purple-500"></i> {{__('Password (Leave blank to keep current)')}}
                    </label>
                ...
                <div>
                    <label for="photo" class="block text-sm font-medium text-gray-700 mb-1">
                        <i class="fas fa-camera mr-1 text-purple-500"></i> {{__('Photo (Current:')}} {{ $employee->photo ? basename($employee->photo) : 'None' }})
                    </label>
                ...
                <div class="flex justify-end space-x-4">
                    <button type="submit" class="inline-flex items-center px-6 py-3 bg-purple-600 text-white font-semibold rounded-lg shadow-md hover:bg-purple-700 transition duration-300 transform hover:-translate-y-1">
                        <i class="fas fa-save mr-2"></i> {{__('Update Employee')}}
                    </button>
                </div>
            </form>
        </div>
